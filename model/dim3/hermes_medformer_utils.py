...
from .trans_layers import TransformerBlock, LayerNorm, Mlp
from .hermes_utils import HierarchyPriorClassifier
...
class BidirectionAttention(nn.Module):
    ...
    def rearrange3(self, x, heads):
        #'b n (dim_head heads) -> b heads n dim_head'
        b, n, l = x.shape
        dim_head = int(l / heads)
        x = x.view(b, n, dim_head, heads).contiguous()
        x = x.permute(0, 3, 1, 2).contiguous()

        return x

    def rearrange4(self, x):
        #'b heads n dim_head -> b n (dim_head heads)', n=N)
    ...
    def forward(self, feat, semantic_map):
        # semantic_map here is flattened and already concatenated with priors

        B, C, D, H, W = feat.shape
        B, N, _ = semantic_map.shape # N = ms*ms*ms + num_priors
        ...
        map_q, map_v = map(lambda t: self.rearrange3(t, self.heads), [map_q, map_v])
        ...
        map_out = self.rearrange4(map_out)
...
class BidirectionAttentionBlock(nn.Module):
    ...
    def forward(self, x, semantic_map):
        # semantic_map here is already flattened and concatenated with priors
...
class SemanticMapGeneration(nn.Module):
    ...
    def forward(self, x):
        ...
        semantic_map = semantic_map.permute(0, 2, 1) # B, map_code_num, map_dim
...
class PriorModule(nn.Module):
    ...
    def __init__(self, prior_dim, task_prior_num=64, modality_prior_num=2, l=10):# 42
        super().__init__()
    ...
    def forward(self, B, tgt_idx, mod_idx):
...
class down_block(nn.Module):
    def __init__(self, in_ch, out_ch, conv_num, trans_num, down_scale=[2,2,2], kernel_size=[3,3,3],
                ...
                act=nn.GELU, map_generate=False, map_dim=None, task_prior_num=42, modality_prior_num=2, l=10):
        super().__init__()
        ...
        if map_generate:
            ...
            # merge the prior selection into the semantic map
            self.prior_selection = PriorModule(prior_dim=map_dim, task_prior_num=task_prior_num, modality_prior_num=modality_prior_num, l=l)
    ...
    def forward(self, x, tgt_idx=None, mod_idx=None):
        ...
        if self.map_generate:
            ...
            ms = semantic_map.shape[1] #number of semantic map tokens
            tn = tgt_idx.shape[1] # number of task prior tokens
            mn = mod_idx.shape[1] # number of modality prior tokens
            
            priors = self.prior_selection(x.shape[0], tgt_idx, mod_idx)
            semantic_map = torch.cat([semantic_map, priors], dim=1) # concatenate the semantic map and prior for latter B-MHA
        ...
        if self.map_generate:
            return out, semantic_map[:, :ms, :], semantic_map[:, ms:, :] #feat, semantic_map, posterior task tokens
        else:
...
class up_block(nn.Module):
    def __init__(self, in_ch, out_ch, conv_num, trans_num, up_scale=[2,2,2], kernel_size=[3,3,3], 
                ...
                map_dim=None, map_shortcut=False, no_map_out=False, task_prior_num=42, modality_prior_num=2, l=10):
        super().__init__()
        ...
        if map_shortcut:
            ...
            self.map_reduction = nn.Identity()
    ...
    def forward(self, x1, x2, map1, map2=None, tgt_idx=None, mod_idx=None):
        ...
        if self.map_shortcut and map2 is not None:
            ...
            ms = semantic_map.shape[1]
            tn = tgt_idx.shape[1]
            mn = mod_idx.shape[1]
            
            priors = self.prior_selection(feat.shape[0], tgt_idx, mod_idx)
            semantic_map = torch.cat([semantic_map, priors], dim=1)
