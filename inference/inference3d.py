...
def inference_whole_image(net, img, tgt_idx, mod_idx, args=None):
    '''
    ...
    with torch.no_grad():
        pred, _ = net(img, tgt_idx, mod_idx)
...
def inference_sliding_window(net, img, tgt_idx, mod_idx, args):
    '''
    img: torch tensor, B, C, D, H, W
    return: prob (after softmax), B, classes, D, H, W
    ...
    classes = tgt_idx.shape[1]
    pred_output = torch.zeros((B, classes, D, H, W)).to(img.device)
        ...
        for i in range(D // half_win_d):
            for j in range(H // half_win_h):
                for k in range(W // half_win_w):
                    ...
                    pred, _ = net(input_tensor, tgt_idx, mod_idx)
